...
# Ⅳ. 연산자
# WEEK 4-1 연산자(사칙, 관계, 논리 연산자)
## 연산자
* 연산자(operator)는 산술 연산자 +,-,* 기호와 같이, 이미 정의된 연산을 수행하는 기호나 키워드를 의미
* 문제를 해결하는 방법에서 도구(장비)와 같은 역할
* C언어는 참과 거짓이라는 상수가 없으며, 0, 0.0, '\0'는 거짓으로, 0이 아닌 모든 값을 참으로 간주
  * ex) (3 && 4) -> 1
* 피연산자
  * 연산(operation)에 참여한는 변수나 값을 피연산자(operand)
## 연산자 분류
### 대입 연산자(assignment operator)
* 변수의 저장 값을 대입하는 =기호가 대입(할당)연산자(assignment operator)
* =연산자 오른쪽 수식을 먼저 계산하고 결과 값을 왼쪽 변수에 대입
* 수식의 결과 값을 변수에 대입하지만 않으면 프로그램에는 영향이 없음
* ex) sum = 10 + 20, sum = add(a,b)
### 관계 연산자(relational operator)
* 관계 연산자는 2개의 피연산자 관계(크기)를 비교하기 위한 연산자
* 관계 연산자가 포함된 수식의 결과는 1(참을 의미)이 아니면 0(거짓을 의미)
### 논리 연산자(logical operator)
* 두 개 또는 하나의 논리값을 0이나 1의 논리값으로 평가하기 위한 연산자
* AND(&&): 둘 다 참이어야 함
* OR(||): 둘 중하나만 참이면 참
* NOT(!): 참이면 거짓, 거짓이면 참
## 연산자 우선순위
![연산자우선순위](https://github.com/jdaun/TIL/blob/master/MOOC/img/priority_operator.PNG)
